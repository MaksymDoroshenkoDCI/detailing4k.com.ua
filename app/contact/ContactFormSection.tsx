...
function ContactFormInner() {
  ...
  useEffect(() => {
    ...
    if (topic === 'podbir-avto') {
      setFormData((prev) => ({
        ...prev,
        message: prev.message.trim().length === 0 ? PODBIR_PREFILL : prev.message,
      }))
    }
    setPrefillDone(true)
  ...
  const isPodbirTopic = searchParams.get('topic') === 'podbir-avto'

  return (
    <>
      <h1 className="text-4xl font-bold mb-4 text-center text-white">
        {isPodbirTopic ? 'Заявка на підбір авто' : 'Контакти'}
      </h1>
      <p className="text-center text-gray-300 mb-12">
        {isPodbirTopic
          ? "Заповніть форму — ми зв'яжемося та узгодимо деталі підбору."
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {submitted ? (
            <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-green-800 mb-2">Запит успішно відправлено!</h3>
